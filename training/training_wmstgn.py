"""WM-STGN training — 20 configs + top-3 ensemble + MC Dropout, optimized for M4 Pro MPS."""
...
from models.stgcn import WMSTGN
...
def train_wmstgn(params, X_tr, Y_tr, X_v, Y_v, adj):
    _, ts, nn_, ic = X_tr.shape
    nh, ntgt = Y_tr.shape[1], Y_tr.shape[3]

    model = WMSTGN(nn_, ic, params["sc"], params["tc"], ts, nh, ntgt, adj,
    ...
    def lr_lambda(epoch):
        ...
        return 0.5 * (1 + np.cos(np.pi * (epoch - warmup_epochs) / (params["ep"] - warmup_epochs)))
...
def run_wmstgn(seq, stats, meta, adj, Y_true_abs):
    print("\n" + "=" * 50 + f"\nWM-STGN TUNING (20 configs on {DEVICE})\n" + "=" * 50)
    ...
    for i, cfg in enumerate(CONFIGS):
        t0 = time.time()
        state, val = train_wmstgn(cfg, X_tr, Y_tr, X_v, Y_v, adj)

        model = WMSTGN(nn_, ic, cfg["sc"], cfg["tc"], ts, nh, ntgt, adj, cfg["ed"], 0.0).to(DEVICE)
    ...
    if ens3_mae < all_maes[best_idx]:
        ...
        final_results = [{"Model": "WM-STGN-ens3", **r} for r in ens3_results]
        final_abs = ens3_abs
    else:
        ...
        final_results = [{"Model": "WM-STGN-best", **r} for r in eval_pm(best_abs, Y_true_abs, meta)]
        final_abs = best_abs

    pd.DataFrame(all_logs).to_csv(f"{PROC}/wmstgn_tuning_log.csv", index=False)
    np.save(f"{PROC}/wmstgn_predictions.npy", final_abs)

    # Save best single model + MC Dropout
    best_cfg = CONFIGS[best_idx]
    state, _ = train_wmstgn(best_cfg, X_tr, Y_tr, X_v, Y_v, adj)
    torch.save(state, f"{PROC}/wmstgn_model_tuned.pt")

    model = WMSTGN(nn_, ic, best_cfg["sc"], best_cfg["tc"], ts, nh, ntgt, adj,
    ...
    np.save(f"{PROC}/wmstgn_pred_mean.npy", mc_abs)
    np.save(f"{PROC}/wmstgn_pred_std.npy", mc_std_abs)
